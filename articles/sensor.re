...
SwitchBotは、SwitchBot Hubを通してCO2センサーがインターネットに接続されていることを前提としています。
...
//image[aws_lambda_create_function_2][関数の作成画面2][scale=0.75]
 ...
 * MACKEREL_SERVICE_NAME：MackerelのAPIキーの設定画面から確認可能
...
SwtichBotから取得したデータが送られていることが確認できます。
...
「次へ」ボタンを選択し、確認画面に遷移し「スケジュールを作成」ボタンを選択します。
...
このように、AWSのLambdaを使うことで簡単にセンサーの値を取得し、いつでもどこでも生育条件を確認することができます。
